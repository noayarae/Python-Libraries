...
### Input
# - 'topog_angle.csv':          CSV file with topographic angles for 0, 45, 90, 135, 270, 225, 270, 315 axis
# - 'stream_data_for_sf.csv':   CSV file with stream data: (0)sb,(1)long_rch,(2)lat_rch, (3)treeh_r, (4)treeh_l, (5)strm_az, (6)strm_w, (7)strm_slen
# - 'SR_data_2019.csv':         CSV file with daily Solar Radiation: (0)Date, (1)julian_d, (2)SR
# - Set time step 't_step' at L.137. It is suggested to set 0.01
...
import get_sun_az_angle as sa_ang
...
### Read SR data
sr_db = []
with open('in03_SR_data_2019.csv', 'r') as file:
    # (1)date, (2)SR
...
n_sbs = 60#len(strm_db)
### For loop through SBs
for sb in range(n_sbs):
    ...
    for dy in range(len(lt_days)):
        ...
        t_step = 0.01#0.01#0.01#0.1 #0.1#1 # 0.1 means 1 hours is divided by 10 #  <-------- SET
    ...
    plt.plot(x_pts, top_pts, color='b', label='Top')
    plt.plot(x_pts, Rtree_pts, color='g', label='R-tree')
    plt.plot(x_pts, Ltree_pts, color='r', label='L-tree')
    plt.plot(x_pts, all_pts, color='k', label='All')
    plt.legend()
    plt.show()
    #'''
print
